...
import { useCurrentRound, useSideBank, useTotalProbability } from '@/src/lib/query';
...
const DesktopBetSection = () => {
	...
	const isEmpty = totalProbability === 0n;

	const pie = useMemo(() => {
		return [
			{
				id: 1,
				value: isEmpty ? 20 : (Number(sideBank[0]) * 100) / Number(totalProbability || 1n),
				color: 'hsl(var(--topaz-bg))',
				borderColor: 'hsl(var(--topaz-border))',
			},
			{
				id: 2,
				value: isEmpty ? 20 : (Number(sideBank[1]) * 100) / Number(totalProbability || 1n),
				color: 'hsl(var(--zircon-bg))',
				borderColor: 'hsl(var(--zircon-border))',
			},
			{
				id: 3,
				value: isEmpty ? 20 : (Number(sideBank[2]) * 100) / Number(totalProbability || 1n),
				color: 'hsl(var(--citrine-bg))',
				borderColor: 'hsl(var(--citrine-border))',
			},
			{
				id: 4,
				value: isEmpty ? 20 : (Number(sideBank[3]) * 100) / Number(totalProbability || 1n),
				color: 'hsl(var(--emerald-bg))',
				borderColor: 'hsl(var(--emerald-border))',
			},
			{
				id: 5,
				value: isEmpty ? 20 : (Number(sideBank[4]) * 100) / Number(totalProbability || 1n),
				color: 'hsl(var(--ruby-bg))',
				borderColor: 'hsl(var(--ruby-border))',
			},
		];
	}, [sideBank, totalProbability, isEmpty]);
